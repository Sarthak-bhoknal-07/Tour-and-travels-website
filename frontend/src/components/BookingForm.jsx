...
// 🚀 COMPONENTS MOVED OUTSIDE TO PREVENT REMOUNTING BUG ON MOBILE
const InputField = ({ label, name, type = "text", icon: Icon, placeholder, value, onChange, ...props }) => (
  <div className="relative group">
    <div className={`absolute left-4 top-1/2 -translate-y-1/2 transition-all duration-300 ${value ? 'opacity-0 scale-50' : 'opacity-40 group-focus-within:opacity-0 group-focus-within:scale-50'}`}>
    ...
    <input
      ...
      autoComplete="off"
      className={`w-full bg-gray-50 border-2 border-gray-100 rounded-2xl py-4 pr-4 transition-all duration-300 outline-none focus:border-brand-gold focus:bg-white focus:shadow-[0_0_20px_rgba(245,166,35,0.15)] text-gray-900 font-bold placeholder:text-gray-400 placeholder:font-medium ${value || type === 'date' || type === 'time' ? 'pl-6' : 'pl-12'}`}
    />
    <label className={`absolute left-4 -top-2.5 bg-white px-2 text-[10px] font-black uppercase tracking-widest text-brand-gold transition-all duration-300 ${value || type === 'date' || type === 'time' ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-2'}`}>
      {label}
    </label>
  </div>
);

const VehicleButton = ({ v, isSelected, onClick }) => (
  <button
    type="button"
  ...
  >
    <div className="flex justify-between items-start mb-1 md:mb-2 relative z-10">
      <span className={`font-black text-lg md:text-xl tracking-tighter ${isSelected ? 'text-gray-900' : 'text-gray-600'}`}>{v.name}</span>
      {isSelected ? (
    ...
    <div className="flex flex-col gap-0.5">
        <div className="text-[10px] font-black uppercase tracking-widest text-gray-400 relative z-10">{v.seats} Seater • {v.ac ? 'AC' : 'Non-AC'}</div>
        <div className="text-brand-gold font-black text-sm relative z-10">₹{v.rate}/km</div>
    </div>
...
export default function BookingForm({ selectedCar = 'ERTIGA' }) {
  ...
  const handleSubmit = async (e) => {
    ...
    const selectedVehicle = vehicles.find(v => v.id === formData.vehicle) || vehicles[0];

    try {
      // 1. Save to Database via Backend
      const backendData = {
        ...formData,
        vehicle: `${selectedVehicle.name} (₹${selectedVehicle.rate}/km)`,
        travelTime: `${formData.travelTime} ${formData.timePeriod}`,
        luggageCarrier: formData.luggageCarrier
      };
      ...
      const templateParams = {
        ...
        vehicle: `${selectedVehicle.name} (${selectedVehicle.seats} Seater, ₹${selectedVehicle.rate}/km)`,
        passengers: formData.passengers.toString(),
        message: formData.message || 'No special request',
        luggage_carrier: formData.luggageCarrier,
      ...
      setFormData({
        ...
        timePeriod: 'AM', vehicle: 'ERTIGA', passengers: 1, message: '', luggageCarrier: 'No'
      });
    ...
    <form onSubmit={handleSubmit} className="bg-white p-6 md:p-12 rounded-[2rem] md:rounded-[2.5rem] shadow-[0_40px_120px_rgba(0,0,0,0.15)] border-t-[10px] md:border-t-[12px] border-brand-gold relative overflow-hidden">
      ...
      <div className="relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
          <InputField label="Full Name" name="customerName" icon={Users} placeholder="Enter your full name" required value={formData.customerName} onChange={handleChange} />
          <InputField label="Mobile Number" name="mobileNumber" type="tel" icon={Phone} placeholder="10-digit mobile number" required value={formData.mobileNumber} onChange={handleChange} />
          <InputField label="Pickup From" name="pickupLocation" icon={MapPin} placeholder="Pickup area or city" required value={formData.pickupLocation} onChange={handleChange} />
          <InputField label="Drop Location" name="destination" icon={MapPin} placeholder="Destination city" required value={formData.destination} onChange={handleChange} />
          <InputField label="Journey Date" name="travelDate" type="date" icon={Calendar} min={today} required value={formData.travelDate} onChange={handleChange} />
          
          <div className="flex gap-2">
            <div className="flex-grow">
              <InputField label="Pickup Time" name="travelTime" type="time" icon={Clock} required value={formData.travelTime} onChange={handleChange} />
            </div>
        ...
        {/* New Luggage Field */}
        <div className="mt-6 md:mt-8 p-4 md:p-6 bg-gray-50 rounded-2xl border-2 border-gray-100">
          <label className="block text-gray-900 font-black mb-4 flex items-center gap-2 text-[10px] md:text-sm uppercase tracking-[0.2em]">
            ...
            {['Yes', 'No'].map((option) => (
              <label key={option} className="flex items-center gap-2 cursor-pointer">
                <input
                  ...
                  name="luggageCarrier"
                  value={option}
                  checked={formData.luggageCarrier === option}
                  onChange={handleChange}
